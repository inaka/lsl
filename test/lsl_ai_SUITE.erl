...
dumb_ai_full_match(_Config) ->
  lsl_test_utils:full_match(lsl_ai_dumb, lsl_match:new(2)),
  lsl_test_utils:full_match(lsl_ai_dumb, lsl_match:new(5)),
  lsl_test_utils:full_match(lsl_ai_dumb, lsl_match:new(10)),
  lsl_test_utils:full_match(lsl_ai_dumb, lsl_match:new(20)),
  {comment, ""}.

-spec rodolfo_start(lsl_test_utils:config()) -> {comment, []}.
rodolfo_start(_Config) ->
  ct:comment("A new match is created"),
  Match0 = lsl_match:new(5),
...
rodolfo_finish(_Config) ->
  ...
  {next, Match2} = lsl_ai_rodolfo:play(Match1),
  [[x], [i, i], [x, i, i]] = lsl_match:snapshot(Match2),

  ct:comment("Now, with just 2 groups, he will reduce the first one"),
  {next, Match3} = lsl_ai_rodolfo:play(Match2),
  [[x], [i, i], [x, x, i]] = lsl_match:snapshot(Match3),

  ct:comment("Now that he has a chance to win, he'll take it"),
  {won, Match4} = lsl_ai_rodolfo:play(Match3),
  [[x], [x, x], [x, x, i]] = lsl_match:snapshot(Match4),

  ct:comment("He will take it also when it means crossing a single stick"),
  Match50 = lsl_match:new(3),
  {next, Match51} = lsl_match:cross(3, 2, 2, Match50),
  {next, Match5} = lsl_ai_rodolfo:play(Match51),
  [[i], [x, i], [i, x, x]] = lsl_match:snapshot(Match5),

  {next, Match6} = lsl_ai_rodolfo:play(Match5),
  [[x], [x, i], [i, x, x]] = lsl_match:snapshot(Match6),

  {won, Match7} = lsl_ai_rodolfo:play(Match6),
  [[x], [x, x], [i, x, x]] = lsl_match:snapshot(Match7),

  {comment, ""}.

-spec rodolfo_full_match(lsl_test_utils:config()) -> {comment, []}.
rodolfo_full_match(_Config) ->
  lsl_test_utils:full_match(lsl_ai_rodolfo, lsl_match:new(2)),
  lsl_test_utils:full_match(lsl_ai_rodolfo, lsl_match:new(5)),
  lsl_test_utils:full_match(lsl_ai_rodolfo, lsl_match:new(10)),
  lsl_test_utils:full_match(lsl_ai_rodolfo, lsl_match:new(20)),
  {comment, ""}.
